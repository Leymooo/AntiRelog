...
public class Settings extends Config {
    ...
    public static class KICK {
        ...
        @Comment("Убивать ли игрока, если он был кикнут во время пвп, и в причине кика есть определённые фразы")
        public boolean      KILL_ON_KICK  = true;

        @Comment({ "Какие фразы должны присуствовать в причине кика, чтобы убить игрока",
                    "Если список пуст, и KILL_ON_KICK = true, то игрока будет убивать при любом кике.", "Фразы писать без цветовых кодов." })
        public List<String> KILL_MESSAGES = Arrays.asList("спам командами");

    }

    @Comment("Кулдавн для обычных золотых яблок. -1 чтобы отключить кулдавн. Работает только во время пвп режима или всегда если пвп отключен")
    public int          GOLDEN_APPLE_COOLDOWN           = 30;

    @Comment("Кулдавн для зачарованых золотых яблок. -1 чтобы отключить кулдавн. Работает только во время пвп режима или всегда если пвп отключен")
    public int          ENCHANTED_GOLDEN_APPLE_COOLDOWN = 60;

    @Comment("Кулдавн для эндер перлов(Жемчуг Края). -1 чтобы отключить кулдавн. Работает только во время пвп режима или всегда если пвп отключен")
